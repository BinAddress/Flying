...
{ 	
  ...
  SPI_InitTypeDef  SPI_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOA, ENABLE);	 //使能PB,G端口时钟
  ...
  SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b;		//发送接收8位帧结构
	...
	NRF24L01_Int_IRQ_Init(); //IRQ 中断初始化
...
void EXTI9_5_IRQHandler(void)
{
 ...
 NRF24L01_Write_Reg(NRF_WRITE_REG + STATUS, state);
		...
		if((state & RX_DR) == RX_DR)
    {
				...
				if(nrf_rx_buf[0] == 0xAA && nrf_rx_buf[1] == 0xAF)
				{
					for(i = 0; i < 32; i++)
					{
						ANO_DT_Data_Receive_Prepare(nrf_rx_buf[i]);
						nrf_rx_buf[i] = 0;
					}			
				}                               
    }
